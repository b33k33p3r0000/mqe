...
Metrics Calculation
===================
Rozsirenene metriky pro hodnoceni strategie.
...
from mqe.config import MIN_HOLD_BARS, STARTING_EQUITY


@dataclass
class MonteCarloResult:
    """
    v9.0 NEW: Vysledky Monte Carlo validace.
...
class MetricsResult:
    ...
    max_drawdown: float
    avg_drawdown: float
    sharpe_ratio: float  # Trade-based (legacy, pro zpetnou kompatibilitu)
    sharpe_ratio_time_based: float  # v12.0 NEW: Time-based Sharpe (realisticky)
...
def calculate_time_based_sharpe(
    ...
    Na rozdil od trade-based Sharpe, tato metoda:
    - Zahrnuje obdobi kdy strategie drzi cash (return = 0%)
    - Dava realisticke hodnoty srovnatelne s industry benchmarky (0.5-2.0)
    - Annualizuje spravne pro hodinova data (sqrt(8760))

    Args:
        trades: Seznam obchodu z backtestu
        price_data: DataFrame s OHLCV daty (1h timeframe, musi mit 'close' sloupec)
        start_equity: Pocatecni kapital
        start_idx: Pocatecni index v price_data
        end_idx: Koncovy index v price_data (None = konec)

    Returns:
        Time-based Sharpe ratio (annualizovany)
    """
    ...
    # Prevod trades na DataFrame pro rychlejsi lookup
    ...
    for _, trade in trades_df.iterrows():
        ...
        try:
            ...
            # Relativni indexy vzhledem k start_idx
            ...
            # Spocitej hourly returns pro tento trade
    ...
    # Annualizace pro hodinova data: sqrt(8760) hodin v roce
...
def calculate_equity_based_sharpe(
    ...
    """
    v13.0 NEW: Equity-based Sharpe Ratio — z dennich equity returnu.

    Na rozdil od time-based Sharpe:
    - Pracuje s dennimi equity zmenami (ne hodinovymi price returns)
    - Spravne reflektuje direction (PnL je jiz direction-aware)
    - Annualizuje sqrt(365) — standard pro krypto
    - Dava intuitivnejsi hodnoty srovnatelne s industry benchmarky

    Returns:
        Equity-based Sharpe ratio (annualizovany)
    """
...
def calculate_sortino_ratio(
    ...
    """
    Sortino Ratio z dennich equity returnu — penalizuje pouze downside volatilitu.

    Konzistentni s equity-based Sharpe: denni returns + sqrt(365).
    """
    if not trades or backtest_days < 30:
        return 0.0
...
def calculate_recovery_factor(total_pnl: float, max_drawdown: float, start_equity: float) -> float:
    """
    v4.0 NEW: Recovery Factor = Net Profit / Max Drawdown (v absolutnich hodnotach)
    """
...
def calculate_metrics(
    ...
    v12.0: Pridan time-based Sharpe ratio (realisticky vypocet)
    v4.0: Pridany Calmar, Sortino, Recovery Factor, Streaks, Monthly analysis

    Args:
        trades: Seznam obchodu
        backtest_days: Delka testovaci obdobi v dnech
        start_equity: Pocatecni kapital
        price_data: (v12.0) DataFrame s OHLCV daty pro time-based Sharpe
        start_idx: (v12.0) Pocatecni index v price_data
        end_idx: (v12.0) Koncovy index v price_data

    Returns:
        MetricsResult s komplentnimi metrikami
    """
    if not trades:
    ...
    # v12.0 NEW: Time-based Sharpe Ratio (realisticky)
    ...
    # Sortino Ratio (daily returns + sqrt(365), konzistentni s equity-based Sharpe)
...
def monte_carlo_validation(
    ...
    Shuffluje poradi obchodu a prepocitava metriky pro odhad
    confidence intervals. Strategie je robustni, pokud vysledky
    zustavaji podobne i pri ruznem poradi obchodu.

    Args:
        trades: Seznam obchodu z backtestu
        n_simulations: Pocet Monte Carlo simulaci (default: 1000)
        start_equity: Pocatecni kapital
        seed: Random seed pro reprodukovatelnost

    Returns:
        MonteCarloResult s confidence intervals a robustnosti
    """
    if not trades or len(trades) < 10:
        # Prilis malo obchodu pro smysluplnou MC simulaci
